...
describe("Try maestro(async (req, res, next) => { next() or Error }) :", () => {
  ...
  describe("Basic functionality:", () => {
    ...
    it("Thenables are not guaranteed to have a `catch` method. This test refers to this.", async () => {
      ...
      const registeringThenable: Promise<void> = new Promise<void>(
        (resolve) => {
          thenable = {
      ...
      const catchingThenable: Promise<void> = maestro(() => thenable)(
        null,
        null,
        next
      );
  ...
  describe("Should invoke `next` with the thrown error:", () => {
    ...
    const route = maestro(async (_req: any, _res: any, _next: any) => {
      throw error;
    });
...
describe("Try maestro.from(RealProblems, (err) => { }) :", () => {
  ...
  it("Handles the error when error is instance of given constructor.", () => {
    ...
    maestro.from(RealProblems, matchedHandler)(
      new RealProblems(),
      req,
      res,
      next
